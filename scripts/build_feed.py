...
def main():
    ...
    args = ap.parse_args()
    data = json.load(open(args.log,"r",encoding="utf-8"))
    ...
    for it in items:
        fe = fg.add_entry(); fe.title(f'[{it["tool"]}] {it["headline"]}'); fe.link(href=it.get("link") or args.site + "/"); fe.pubDate(email.utils.format_datetime(datetime.datetime.strptime(it["date"], "%Y-%m-%d"))); fe.description(f'{it["severity"]} — {it["impact"]}')
